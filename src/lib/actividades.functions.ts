...
export const listarChileCulturaPorRegiones = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{
    ...
    const t0 = Date.now();
    // normalize alias forzarRecarga -> force
    const force = data.forzarRecarga ?? data.force ?? false;
    const inputRegiones = (data.regiones ?? [13]).filter((n) => Number.isInteger(n) && n >= 1 && n <= 16).slice(0, 6);
